require_relative 'a01_practice_enums'
...
def factors(num)
  ...
  1.upto(num) { |x| factors << x if num % x == 0 }
  ...
  def bubble_sort!(&prc)
    ...
    until swapped == false
      ...
      self.each_index do |idx1|
        idx2 = idx1 + 1
        break if idx2 >= self.length

        if prc.call(self[idx1], self[idx2]) > 0
...
def substrings(string)
  ...
  (0...string.length).each do |start_idx|
    (start_idx...string.length).each do |end_idx|
      substrs << string[start_idx..end_idx]
    end
  end
...
def subwords(word, dictionary)
  ...
  candidates.each do |candidate_word|
    real_words << candidate_word if dictionary.include?(candidate_word)
  end
...
class Array
  ...
  def my_inject(&blk)
    ...
    self[1..-1].my_each { |el| accumulator = blk.call(accumulator, el) }
    accumulator
  end
end
